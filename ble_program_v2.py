    ...
    def write_to_csv(self,data_values:[Any]):
        ...
        with open(self.path, "a+") as f:
            if os.stat(self.path).st_size == 0:
                ...
                f.write(f"{datetime.now()},{data_values.ax},{data_values.ay},{data_values.az},\n")
...
class Connection:
    ...
    def notification_handler(self, sender,data: Any):

        # self.rx_data.append(int.from_bytes(data, byteorder="big"))
        # self.record_time_info()
        # if len(self.rx_data) >= self.dump_size:
        self.ax = (struct.unpack('<f', bytes(data[0:4])))[-1]
        self.ay = (struct.unpack('<f', bytes(data[4:8])))[-1]
        self.az = (struct.unpack('<f', bytes(data[8:12])))[-1]
        self.data_dump_handler(self)
        print(self.ax)
        self.clear_lists()

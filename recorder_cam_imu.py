...
class NanoIMUBLEClient(object):
    ...
    def __init__(self, uuid:str, imu:int) -> None:
        super().__init__()
        self._client = None
        self._device = None
...
async def run_imu():
    ...
    imu_client_1 = NanoIMUBLEClient(IMU_UUID, 1)
...
async def run_imu_2():
    ...
    imu_client_2 = NanoIMUBLEClient(IMU_UUID1, 2)
    # await imu_client_1.connect()
    await imu_client_2.connect()
...
def between_callback2():
    ...
    loop.run_until_complete(run_imu_2())
    loop.close()
...
if __name__ == "__main__":
    ...
    t3 = threading.Thread(target=between_callback2)
